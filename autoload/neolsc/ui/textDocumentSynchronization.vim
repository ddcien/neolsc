...
function! neolsc#ui#textDocumentSynchronization#didChange() abort
    ...
    if l:buf_ctx['_tick'] == nvim_buf_get_changedtick(l:buf)
        return
    endif
    ...
    let l:new = nvim_buf_get_lines(l:buf, 0, -1, v:true)
    let l:contentChanges = s:_build_TextDocumentContentChangeEvent(l:sync_kind, l:old, l:new)

    call neolsc#lsp#textDocumentSynchronization#didChange(l:server, l:buf, l:contentChanges)

    let l:buf_ctx['_tick'] = nvim_buf_get_changedtick(l:buf)
