...
function! s:TextEdit_Location_compare(edit0, edit1) abort
    let l:ret = neolsc#ui#utils#position_compare(a:edit0.range.start, a:edit1.range.start)
    if l:ret != 0
        return l:ret
    endif
...
function! s:handle_documentChanges(documentChanges) abort
    for l:documentChange in a:documentChanges
        if has_key(l:documentChange, 'kind')
            ...
            call s:handle_documentChange(l:documentChange)
        endif
    endfor
endfunction
...
function! s:handle_changes(changes) abort
    for [l:uri, l:edits] in items(a:changes)
        call s:handle_uri_edits(l:uri, l:edits)
    endfor
endfunction

function! s:handle_documentChange(documentChange) abort
...
function! s:handle_uri_edits(uri, edits) abort
    if empty(a:edits)
        return
    endif

    let l:path = resolve(neolsc#utils#uri#uri_to_path(a:uri))
...
" public {{{
function! neolsc#ui#edit#WorkspaceEdit(buf, workspaceedit) abort
    ...
    if has_key(a:workspaceedit, 'changes')
        call s:handle_changes(a:workspaceedit.changes)
    elseif has_key(a:workspaceedit, 'documentChanges')
        call s:handle_documentChanges(a:workspaceedit.documentChanges)
    else
        return
    endif
...
function! neolsc#ui#edit#TextEdits(buf, textedits) abort
    let l:cur_view = winsaveview()
    call s:handle_buf_edits(a:buf, a:textedits)
    call winrestview(l:cur_view)
endfunction
" }}}
